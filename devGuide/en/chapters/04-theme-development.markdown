...
Liferay provides a well organized, modular structure to its themes, so you can
quickly modify objects on the page. You can customize every aspect of the theme
or make only minor alterations (e.g., portlet window borders), with your theme
inheriting the styling, images, and templates from any of the built in themes.
Overriding built in themes only where necessary saves you time and keeps your
themes smaller and less cluttered, since you don't need to duplicatively include
data that already exists in the default theme (e.g., emoticons graphics for the
...
the HTML that controls the page design. 
...
- **Velocity:** Create templates and customize the HTML generated by the theme.

- **JavaScript:** Add special behaviors to your theme. 

- **XML:** Some theme settings are specified in XML.

To follow the examples in this guide you also need some familiarity
using either the command line or the Liferay IDE. 
...
Studio first, then with the terminal. 
...
theme is empty, but once you run `ant deploy`, all the files and folders of the
theme will be populated.
...
contain very limited styling. Take advantage of an *existing* theme's styling by
setting the theme of your choice as the base for your theme theme.
...
To specify a base theme, edit the `build.xml` file, changing *_styled* in
...
1. Go to your web browser and login to the portal. 
...
must mirror the parent theme's directory structure. Since you're only going to
customize the parts of your theme that will differ from the parent theme, place
only the folders and files you need to customize there. 

The several other folders inside `docroot` were copied over from the parent
theme in your Liferay bundle when you deployed your theme. Use these files as
the basis for your modifications. For example, to customize the navigation, copy
...
place a single file there called `custom.css`, and define all of your new styles
here. Because `custom.css` is loaded last, styles defined here override any
styles in the parent theme.

It's best practice to make your custom theme styles using only the
`custom.css` file, not overriding any of the templates. This makes future
upgrades easier, because you won't have to manually modify your templates
to add support for new Liferay features. 
...
---
 ...
 directly in your Liferay bundle. Our theme's `custom.css` file is located
 in
 `liferay-portal-[version]/tomcat-[tomcat-version]/webapps/deep-blue-theme/css`.
 See the Liferay wiki for more information about Lifersay Developer Mode. 
...
YUI3 framework. You can take advantage of Alloy or YUI3 in your themes. Inside
...
You can define settings to make your theme configurable. You'll need to create a
file named `liferay-look-and-feel.xml` inside the `WEB-INF` directory, with the
following content:
...
Let's say we want to be able to choose from two different page headers (perhaps
one includes more details, while the other is smaller). Instead of creating two
themes that are identical except for some changes in the header, let's create
one and define a setting that let's us choose which header is displayed. 

In the `portal_normal.vm` template we'll write:
...
Then, we write two different entries in the `liferay-look-and-feel.xml` file
that refer to the same theme, but have a different value for the header-type
setting:
...
portal. Using *configurable* settings, you can let users turn certain theme
features on or off, or allow users to provide input to a theme setting. 

As an example, you can create an option to display a slogan next to
...
When the portal administrator saves the settings, your site's pages display the
new footer, including the slogan. 
...
![Figure 4.6: The slogan displayed in the page footer.](../../images/themes-custom-configurable-setting-displayed.png)
 ...
 to properly display configurable theme settings, like the slogan text area and
...
Specify color schemes with a CSS class name, which also lets you choose
...
file in it for each color scheme. In the case above, we can either have one file
...
navigation. This class will be named in the following pattern:
...
inherit its styling from. In addition to the *_styled* theme, you can choose to
inherit from the *_unstyled* theme, which contains no styling. This makes more
work for you, but offers full flexibility to design your CSS files from scratch. 
from scratch. 
