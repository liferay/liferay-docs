...
completely that it would be difficult or impossible to tell that the site is
running on Liferay. 
...
To follow the examples in this guide you also need some familiarity with
...
Here's a snapshot of your *Package Explorer* located in Developer Studio. 
...
After redeploying your theme, it will have a thumbnail preview just like the
*Classic* theme. 
...
portlet borders and bullet styles, but this number may grow in the future.
Modify these settings from the `liferay-look-and-feel.xml` file.

If set to `false`, the portal's default behavior is to turn off portlet borders.
The default is `true`. 

Here's the XML when you modify the default to `false`:
