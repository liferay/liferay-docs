...
like to use dependency injection. Finally, if you intend to create a
service-oriented architecture for your custom services, you'll have to create
the interfaces for your Impl-classes to implement that Service Builder
auto-generates. You'll also have to manually keep your interfaces and
Impl-classes in sync. Next, let's roll up our sleeves and learn how to use
Service Builder.
...
plugins should use separate namespaces and you shouldn't use a namespace already
claimed by Liferay (such as `Users` or `Groups`).
