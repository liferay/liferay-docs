...
first released version of Liferay 6.2. In Liferay 6.2.0 M5 and prior versions,
...
Each of these methods returned URLs generated by the server that included
authentication tokens (the `p_auth` or `p_p_auth` URL parameters) whenever
necessary. However, that behavior posed a security risk. Please see
[LPS-34098](https://issues.liferay.com/browse/LPS-34098) for details. As of
...
2. If you find no occurrences, you don't need to update your plugins. In this
...
2. Refactor your code to remove dependencies on the `Liferay.PortletURL` object
   ...
   `portlet.url.generate.by.path.enabled=false`. Let's examine two examples:
    ...
    In example 1, we update our code so that instead of using the
    `Liferay.PortletURL` object to create a `portletURL`, we use the
    `portletULRFactory` to create a `LiferayPortletURL` object.
    ...
    In example 2, we again update our code to avoid using the
    `Liferay.PortletURL` object. Instead, we use the
    `<liferay-portlet:renderURL />` tag. Note that in example 2, we have to add
    the `namespace` for every parameter manually.
