...
management, organizations, page building and content management). You can
...
portlets (and any other plugin types) a breeze. All you need to do is
...
![tip](../../images/tip-pen-paper.png)**Tip:** Liferay
...
copying a file into a folder and Liferay's autodeploy mechanism takes
advantage of that ability. You should be aware though that some
...
Select the *Sample* category, and then click *Add* next to *My Greeting*.
Your portlet should appear in the page below.
...
for each page in the portlet. By default, MVCPortlet
uses a JSP with the mode name for each of the registered
...
that appears at the top of the page to all logged in users).
...
file. If a portlet plugin has dependencies on
...
free of global tags such as `<html>` or `<head>`.
...
`portlet.xml` options, there are optional Liferay-specific
...
-	**edit.jsp** will show a form with a text field allowing the greeting to be
	changed, along with a link back to the view page.
...
![tip](../../images/tip-pen-paper.png)**Tip:** If your
...
`<portlet:renderURL>` tag, which is defined by the
`http://java.sun.com/portlet_2_0` tag library. These URLs have
...
so the URL must always go to the portal responsible for
...
`aui`, signifying that it is part of the Alloy UI tag library.
...
`<portlet:defineObjects/>`. The portlet specification defined
...
setting preferences from a JSP, because they are executed in what is
...
good news is that once you get acquainted with them, they become simple and
useful.
...
action, it would have no option but to repeat it over and over to
...
-   **Action phase**: The action phase can only be invoked for one
    portlet at a time and is usually the result of a user interaction
...
-   **Render phase**: The render phase is always invoked for all
    ...
    rendered before those with a lower weight.
...
to create a subclass of `MVCPortlet` or create a subclass of
`GenericPortlet` directly (if you don't want to use Liferay's lightweight
framework).
...
Finally, you will need to make a minor change in the `edit.jsp` file and
change the URL to which the form is sent in order to let the portal know
to execute the action phase. This is the perfect moment for you to
...
-   *resourceURL*: This type of URL can be used to retrieve images, XML,
    ...
    make AJAX requests to the server. The key difference between this URL
    type and the other two is that the portlet has full
    control of the data that will be sent in response.

So we must change the `edit.jsp` to use an *actionURL* by using the JSP
tag of the same name. We'll also remove the previous code that was saving
the preference:
...
Try deploying the portlet again after making these changes; everything
...
let her know that the preference has been saved right after clicking the
...
render phase. The first way is through render parameters. Within your
implementation in the `processAction` method you can invoke the
`setRenderParameter` method to add a new parameter to the request that the
...
phase (that is within the `processAction` method). In order to pass parameter
...
I mentioned there was a second way and in fact it is better
...
later executions of the portlet until the portlet is invoked with
...
portal will render our portlet using the render parameters that we initially set.
...
The second way of passing information from the action phase to the
...
to do this operation easily. In the `processAction` method you need to use the
...
![Figure 3.x: The sample "My Greetings"? portlet showing a success message](../../images/portlet-greeting-save.png)*Illustration 1: The sample "My Greetings" portlet showing a success message*

There is also an equivalent utility class for notifying errors. This is
...
![Figure 3.x: The sample "My Greetings" portlet showing an error message](../../images/portlet-invalid-data.png)*Illustration 2: The sample "My Greetings" portlet showing an error message*
...
the `edit.jsp` modify the `actionURL` so that it looks like this:
...
The next section explains an extension to the
portlet specification that Liferay provides by Liferay generate pretty URLs to your portlets that you
can use.
...
Since Liferay 6 there is a feature that requires minimal work to change
this into:
...
URL mapping, you can check full discussion of this topic in
*Liferay in Action*.
