...
Let's make our portlet do something useful. First, we'll give it have two pages:
...
an action on within a portlet, each of the page's portlets are rendered anew.
...
operation, the portal would also invoke the navigation portlet with its default
view. 
...
- *actionURL*: Execute an action phase before rendering all the portlets in the
  page. 
