...
- Service Builder helps developers save time, both initial development time and
  maintenance time (just create service.xml, run SB, and add custom methods to
...
the need to write and maintain database access code. The code Service Builder
generates is database-agnostic, as is Liferay itself--in fact, all of Liferay's
services have been generated with Service Builder. To generate a basic service
layer, you only need to create a `service.xml` file and run Service Builder.
This generates a new service `.jar` file for your project. The generated service
`.jar` file includes a model layer, a persistence layer, a service layer, and
related infrastructure. Service Builder uses Spring for dependency injection and
Hibernate for object-relational mapping, but it hides the complexities of using
these technologies. The remote services Service Builder generates include SOAP
utilities and can be accessed via SOAP or JSON. Although the number of files
Service Builder generates can seem intimidating at first, developers only need
to work with a few files in order to make customizations to their applications
and add business logic.

Each entity generated by Service Builder contains an implementation class in its
service layer that the developer customizes. Business logic is implemented here,
and it is also responsible for calling the persistence layer to retrieve and
store data entities. Optionally, this layer can have two services: a local
service layer and a remote service layer. The local service contains the
...
the code necessary to allow access to the remote services. 

Service Builder allows multiple portlet applications to share the same
underlying database code. In a standalone web application, the database access
code can be bundled with the rest of the application. However, in a portal, web
applications are often composed of multiple portlets that need to access the
same data and the same application logic. Separate web applications (i.e.,
separate `.war` files) have separate class loaders, and the Java EE
specification prohibits sharing data and application logic across class loader
boundaries. Service Builder allows different portlets to access the same data
and application logic by creating an underlying framework that's designed to
support a portal environment. 

<!-- I think we have a straw man in the paragraph above. In the circumstance
you describe, I would recommend developers simply bundle multiple portlets into
the same application. -Rich -->

The code generated by Service Builder exists in a single plugin but can
...
service layer, Service Builder generates a new service `.jar` file with which
you can replace the old one without needing to restart your application server.

<!-- This is not true. If you're sharing a service .jar on the global classpath
and you need to update it, you have to shut the server down, copy the new .jar
over the other one, and then start the server again. If you're now talking about
hot deploying your app again after regenerating the services, that's different,
but it's not clear from the above paragraph, as there's no transition between
the one discussion about sharing services and the other one about updating
services easily. 

I recommend removing everything in this intro about sharing services among
plugins. It introduces complexity to the discussion which is unnecessary at this
point, when a developer is just trying to get his head around a new technology.
It's better to make a section about it toward the end of the chapter, after the
reader already understands Service Builder. -Rich -->
...
<!-- Recommend deleting the above paragraph. -Rich -->
...
<!-- I don't believe the class loader issue is a compelling reason to use
Service Builder over competing technologies. In fact, it's a non-issue. I could
write a straight Hibernate service layer, .jar it up and stick it on the global
classpath and get the same benefit for multiple applications. Compelling reasons
to use Service Builder include shorter development time (which you've already
mentioned), proper layering of your application, integration with Liferay,
built-in caching, simple, auto-generated finders, and the ability to define your
own queries using Dynamic Query or your own SQL. -Rich -->
...
choice, such as JPA or Hibernate. If you're inclined to take this
approach, just remember that you'll have to independently solve the problem of
sharing services across class loaders. <!-- Not compelling. -Rich -->You'll also have to implement your own
object-relational mapping solution as well as Spring configuration, if you'd
like to use Spring for dependency injection. <!-- Also not compelling. -Rich -->Finally, if you intend to use a
...
have automatically generated for you. <!-- Compelling. -Rich -->You'll also have to manually keep your
interfaces and implementation classes in sync. <!-- Not compelling; this is
standard Java. Plus, you've never mentioned it before; it sounds like it's just
thrown in there. -Rich -->Next, let's roll up our sleeves
and learn how to use Service Builder. 

<!-- Well, I think the above essay needs some work. From what I could identify,
the only compelling reasons to use Service Builder over something a developer
already knows is that it's purportedly faster to get up and running (which might
not be the case, since I'm unfamiliar with it), and I can get remote services
generated for me somehow (but it's optional, so some manual intervention must be
required). While as a developer I might be interested in those two things, the
essay then goes on to mention a whole bunch of things that aren't compelling to
me, and it goes on for a long time, even repeating itself.  By the time I'm
finished, I've forgotten the compelling reasons (faster development time and
remote services) I was given for using Service Builder, and I'm not sure I trust
the author, because he gave me so many uncompelling reasons for using Service
Builder.  The only conclusion I as a developer can reach is I like what I use
now; it's worked well for my other projects; I like keeping my code consistent,
so I think I'm going to skip this chapter and keep using whatever I'm using now. 

Maybe what we can do is approach this the old fashioned way: as a five-part (or
multiple-part, if you have more supporting arguments) essay. Outline it first,
including the thesis and the arguments with their supporting facts, and then
write the essay based on the outline, with an intro and a concluding paragraph.

If you need a reminder, here's a good site that can provide those of us out of
college for 2 decades with some help: 
