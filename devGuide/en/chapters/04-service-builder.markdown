...
Service Builder solves an important portal problem with regards to class
loaders. In a standalone web application, the database access code can be
bundled with the rest of the application. However, in a portal, web applications
are often composed of multiple portlets that need to access the same data and
the same application logic. Separate web applications (i.e., separate `.war`
files) have separate class loaders and it's normally difficult to share data and
application logic across class loader boundaries. Service Builder allows
different portlets to access the same data and application logic by creating an
underlying framework that's designed to support a portal environment. 
...
service layer, Service Builder generates a new service `.jar` file. You can
hot-deploy it replacing your application's old `.jar` file without restarting
...
(EJBs) for everything. Making even trivial changes to the applications, via
their EJBs, often required editing many XML configuration files and Java
classes. Thankfully, Service Builder has evolved since then to use Spring and
Hibernate, gaining a lot a flexibility and allowing changes to be made very
simply--in many cases, all that's required is modifying the `service.xml` file
and re-running Service Builder. Liferay isn't a generic code generation tool;
it's specifically designed to meet the needs of Liferay and Liferay plugin
...
approach, just remember that you'll have to solve the problem of sharing
services across class loaders. You'll also have to implement your own
...
create interfaces for your implementations, when you could have generated them
with Service Builder. You'll also have to manually keep your interfaces and
implementation classes in sync. Next, let's roll up our sleeves and learn how to
use Service Builder. 
...
location entity represents a location at which a social event is held. Since an
event must have a location, the event entity will reference a location entity as
...
portlet itself. After we finish creating the project and our first portlet,
we'll use the Liferay portlet wizard to create our Location Listing portlet in
this project. Here's some key information to specify for the Event Listing
portlet project in the first wizard: 
...
`com.nosester.portlet.eventlisting` for the package path. This makes sure that
the service classes will be generated in the `com.nosester.portlet.eventlisting`
...
an entity. In this case, the combination of columns makes up a compound key for
the entity.
...
to preserve the finders you created. 
...
[http://www.liferay.com/documentation/liferay-portal/6.1/development](http://www.liferay.com/documentation/liferay-portal/6.1/development).
But we've also listed `service.xml` content here for your convenience. Note,
we've added some comments to highlight the service's various elements. Besides
that, your `service.xml` file's contents should look similar to it: 
...
generated, you should never modify it, or your changes will be overwritten the
next time you run Service Builder. Instead, all custom code should be placed in
...
description, and location *name*. If you recall, our event entity refers to it's
location by the location's ID, not its name. That is, the event entity table,
`Event_Event`, refer's to an event's location by its long integer ID in the
...
the following finder method and static field to the `EventFinderImpl` class:
    ...
    public List<Event> findByEventNameEventDescriptionLocationName(
        String eventName, String eventDescription, String locationName, int begin,
        int end) {
...
Each model has `field` elements that represent their model entity's columns. And
...
entity's columns in your project's `serivce.xml` file. Service Builder generated
...
you not only can influence how your model's input fields are displayed but you
can also set their database table column sizes. And you have your choice of how
to apply hints. You can insert individual hints directly into your fields, you
can apply a set of default hints to all of a model's fields, or you can define
collections of hints to apply at either of those scopes. Looks like you've
picked up on the "hints" on how Liferay model hints help portlet data! 
...
data model as entities to use in services. We used Liferay IDE's powerful
editing modes to create services, relate service entities, and generate our
implementation stubs. We implemented our business logic and relied on Service
Builder to generate the corresponding interfaces. We found out that integrating
our own SQL queries in our services was easy and that model hints can easily
help us specify service entity data limitations and entity display
customizations. And we merely scratched the surface of remote services. 

In the next chapter, we'll show you how to find other services and invoke remote
