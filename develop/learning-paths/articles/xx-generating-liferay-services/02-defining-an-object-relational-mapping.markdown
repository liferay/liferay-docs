...
to explain how to use Service Builder, we'll refer to a sample application
...
We'll use the event-listing-portlet project to manage and list these events and
locations. We need to add some entities, or model types, to represent our
organization's events and the locations. We'll define two entities called
*events* and *locations*. The event entity represents a social event that can be
scheduled for an organization, while the location entity represents a location
at which a social event can take place. Since an event must have a location, the
event entity will reference a location entity as one of its attributes. 

![Figure 5.1: The Event Listing Portlet lets you add and modify social events. The portlet relies on its event and location entities and the service infrastructure that was generated by Liferay Service Builder.](../../images/service-builder-view-events.png)
...
files in the `com.liferay.docs.portlet.eventlisting` package. We'll add some
