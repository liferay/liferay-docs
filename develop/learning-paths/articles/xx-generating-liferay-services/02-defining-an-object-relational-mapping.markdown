...
to demonstrate how to use Service Builder, let's build a sample application
consisting of two portlets: the Event Listing portlet and the Location Listing
portlet. These portlets allow users to create, read, update, and delete custom
locations and events which can take place at these locations. An organization
can use the Event Listing and Location Listing portlets to schedule events.
We'll use the event-listing-portlet project to manage and list these events. We
need to add some entities, or model types, to represent our organization's
events and the locations where they hold their events. We'll define two
entities: *events* and *locations*. The event entity represents a social event
that can be scheduled for an organization, while the location entity represents
...
- **Package path:** *com.liferay.docs.portlet.eventlisting*
...
service classes are generated in the `com.liferay.docs.portlet.eventlisting`
...
`docroot/WEB-INF/service/com/liferay.docs/portlet/eventlisting` and
`docroot/WEB-INF/src/com/liferay.docs/portlet/eventlisting`, respectively. Please
...
`/docroot/WEB-INF/service/com/liferay/docs/portlet/eventlisting/service/persistence`
folder and the `-PersistenceImpl` classes found in your
`/docroot/WEB-INF/src/com/liferay/docs/portlet/eventlisting/service/persistence`
folder.
...
that, your `service.xml` file's contents should look similar to this: 
    ...
    <service-builder package-path="com.liferay.docs.portlet.eventlisting">
