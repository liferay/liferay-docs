...
Screenlet in their app. The Screenlet class can contain attribute fields for 
...
following Screenlet XML in `activity_main.xml` when inserting Login Screenlet: 
...
respectively. The Screenlet class then uses these attributes to control the 
Screenlet's behavior. This gives the app developer control over how the 
Screenlet functions in their app.
...
This code defines the attributes `layoutId`, `groupId`, and `autoLoad`. You'll 
add functionality to these attributes in the Screenlet class. Here's a brief 
listing of what each needs to do: 
...
`getguestbooksscreenlet` package. Edit the class declaration to extend Screens's 
...
the `groupId` and `autoLoad` attribute fields. For constructors, you can use the 
...
The `onItemClicked` implementation notifies the listener of the Guestbook the 
user clicked in the `ListView`. 
...
site ID as needed. Add these methods as follows:
...
`guestbooks_attrs.xml`. Add the method now as follows: 
    ...
    protected View createScreenletView(Context context, AttributeSet attributes) {
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attributes, R.styleable.GetGuestbooksScreenlet, 0, 0);

        int layoutId = typedArray.getResourceId(R.styleable.GetGuestbooksScreenlet_layoutId, getDefaultLayoutId());
...
setting. If the app developer hasn't set `layoutId`, you instead use 
`getDefaultLayoutId()` to return the Screenlet's default `layoutId`. You then 
set `_autoLoad` and `_groupId` similarly. If the app developer hasn't set the 
attribute, you use the second argument of the `TypedArray` method to specify a 
...
one in the Screenlet's XML. You finish the `createScreenletView` method by 
inflating the layout specified in `layoutId` and returning its view. 

Now you need to implement the `createInteractor` method of `BaseScreenlet`. This 
is a factory method in which you must create the Interactor for a specific 
action, passing in the Screenlet's ID. The `getScreenletId()` method returns 
this ID. Add `createInteractor` as follows:
...
Since there's no other UI button or action the user can take to load the 
...
listener of the guestbook the user clicks in the UI. Replace the *TODO* with the 
following code: 
