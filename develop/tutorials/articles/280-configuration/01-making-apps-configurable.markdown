...
Automatic UI generation is pretty cool, but you might be wondering how to read
...
3.  The `activate()` method uses the method `ConfigurableUtil.createConfigurable()`
    to convert a map of properties with the configuration to a typed class,
    which is easier to handle.
4.  The configuration is stored in a `volatile` field. Don't forget to make it
    `volatile` or you'll run into weird problems relating.
    <!-- Maybe we can explain this better?-->
...
<!-- Breakpoint for new tutorial? -->

## Accessing Your Configuration in a JSP Portlet Application [](id=accessing-your-configuration-in-a-jsp-portlet-application)

It's very common to read a configuration from a portlet class. If the portlet is
a JSP portlet, the configuration object can be added to the request object so
that configurations can be read from the JSPs that comprise the application's
view layer. This section shows how to read a configuration from a portlet class,
add it to the request, and read it from the view layer. The import statements
are included in the code snippets so that you can see the fully qualified class
names of all the classes that are used.
...
The full `init.jsp` file for this example has these contents:
...
It's a Liferay convention that all JSP imports go in an `init.jsp` file. All
other JSPs import `init.jsp`. This convention ensures that you only have to
manage JSP dependencies in a single file.

Next, obtain the configuration object from request object and read the desired
configuration value from it. Here's a `view.jsp` file that does this:
...
The word <font color="blue">*blue* is written in blue text. Note that *blue* is displayed by
default since you specified it as the default in your `ExampleConfiguration`
interface. If you go to *Control Panel* &rarr; *Configuration* &rarr; *System
Settings* &rarr; *Other* and click on the *Example configuration* link, you can
find the `Favorite color` setting and change its value. The JSP reads the
configuration, and refreshing the UI reflects this update.
...
after Other. Other will always be shown last. To specify a category, use the
`@ExtendedObjectClassDefinition` annotation as in the following example:
<!-- I think the above might need to be updated? The order right now is WCFF,
then Other, so it doesn't seem alphabetical-->

    @ExtendedObjectClassDefinition(category = "foundatiop")
    @Meta.OCD(
        factory = true,
...
a framework called the Configuration API that is based on the standard OSGi
Configuration Admin API shown in the previous section.
...
need a new dependency to use it. However, its implementation is
distributed as a module called `portal-configuration-module-configuration`, so
you will need to make sure it is installed in order to use it.
...
`ConfigurationProvider`. Here's how you obtain a reference to it:
...
value. In future releases, it may have more purposes.
<!-- Does it? -->
