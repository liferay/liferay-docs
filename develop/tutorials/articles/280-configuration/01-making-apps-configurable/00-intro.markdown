...
certain aspects of the application, within bounds set by the developer. You're
free to set up application configuration using any technology, but Liferay's
configuration framework simplifies the task by auto-generating a UI with no
effort beyond defining the configuration options in a Java interface. Complete
...
3.  Additional tutorials will be written on developing configuration screens in
    the user interface for scopes that don't have auto-generated UIs (for
    example, Instance Settings and portlet instance configuration screens).
...
autogenerated in the System Settings application. By default, configurations are
placed in Platform &rarr; Third Party. Make it easier to find for your
