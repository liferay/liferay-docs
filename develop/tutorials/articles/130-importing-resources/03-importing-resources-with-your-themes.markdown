...
- **web content (based on structure and template):** Edit the article, click
  *Download* to download it as a file `article.xml`. Create a folder for the
  ...
  [Features of Liferay's WCM](/docs/6-2/user/-/knowledge_base/u/features-of-liferays-wcm#rich-wysiwyg-editing)
...
5.  In your `liferay-plugin-package.properties` file, include
    ...
    list and set `resources-importer-developer-mode-enabled=true`:
...
7.  Comment out the `resources-importer-target-class-name` property to import 
    into a site template or set it to `com.liferay.portal.model.Group` to 
...
8.  Deploy your plugin into your Liferay instance.
