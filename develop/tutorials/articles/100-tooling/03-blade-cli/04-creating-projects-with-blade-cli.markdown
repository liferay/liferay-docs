...
- `-h, --host-bundle-bsn <String>`: If a new JSP hook fragment needs to be
  created, provide the name of the host bundle symbolic name. This is required
...
- `-v, --liferay-version`: The version of @product@ to target when creating a
  project. The default is `7.0`.
...
the `-v` flag. This lets you specify the @product@ version you want to build
your project for. Add the `-v 7.1` to your `create` command sequence to do this.

<!-- TODO: The below text should replace the text above once 7.1 GA is out. -Cody

If you want to generate a project for a previous version of @product@ (e.g.,
