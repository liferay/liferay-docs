...
by any application with web access to your server: all the application needs is
permission. Remote services are published as JSON or SOAP (using a
...
To create web services for the Guestbook application takes two steps: 

1.  Generate the web services with Service Builder. 

2.  Expose the services you want and wrap them in permission checks. 

There's a certain level of security in the assumption that local services can
only be called by other services in the container. For example, the web app does
...
You may be interested to know that Service Builder just did absolutely nothing.
