...
(JSPs, FreeMarker templates, etc.). Remember that when invoking a service, you
should always call one of the static service methods of a `*Util` class. You
should never call the service methods of an `*Impl` class directly.

In the Microblogs application, for example, the Microblogs portlet needs to
perform create, read, update, and delete (CRUD) operations on Events. To
