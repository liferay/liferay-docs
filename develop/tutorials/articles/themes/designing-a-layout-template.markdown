...
on which you can design a layout template. If this seems overwhelming, don't
...
In case you're wondering what the source code of a Liferay Layout Template looks
like, here's the content of Liferay's *1-2-1 Columns Layout CE* TPL file,
...
You can refer to the *1-2-1 Columns Layout CE* source as you design your own
layout template. 

You can follow these steps to design your template: 
...
`.tpl` file in the `docroot` folder. 
...
`id=main-content` and `role=main`. A layout template with a class name
...
3.  Within the layout template `<div>`, specify a `<div>` for each of row
...
supports two rows of portlets, add two `<div>`s, and so on.
...
supports only one column, add one column `<div>`. If it supports two columns, add
two column `<div>`s, and so on.

    Each column `<div>` column must have a unique CSS ID attribute. A common
    convention is to start each ID with *column-*, followed by an integer
    representing where that column is sequentially located in the template. For
    example, you could name the first column `id="column-1"`, you could name
    the second column `id="column-2"`, and so on. 
    ...
    *portlet-column-first*, *portlet-column-last*, or *portlet-column-only* CSS
    class respectively. CSS class values for a column `<div>` must be space
    separated. The column `<div>` elements in the *1-2-1 Columns Layout CE*
    source above, provides a good example of specifying the various kinds of
    column `<div>`s. 
    ...
    *width* value of each *span[width]* must be an integer value between 1 and
    ...
    the last column of four equal width columns in a row: 
...
![tip](../../images/tip.png) **Tip:** To save time, you can copy an
existing layout template and modify it to fit your needs. 
