...
service layer, persistence layer, and a few configuration files. Don't worry about the number of generated
...
To understand the code generated by service builder we will first take a moment to investigate `service.xml` and the directory structure it creates. Note that the files listed under Local Service and Remote Service below are only generated for an entity that has both `local-service` and `remote-service` attributes set to `true`. Service Builder generates code for these entities in the following two locations. 

- `/[PROJECT_NAME]-api/src/main/java/[YOUR]/[PACKAGE]/[PATH]`
- `/[PROJECT_NAME]-service/src/main/java/[YOUR]/[PACKAGE]/[PATH]`

These locations are formed using the `package-path` attribute contained in the `<service-builder>` element in `service.xml`. For example, the Liferay's Bookmarks module has generated code in the following two locations.

- `/bookmarks-api/src/main/java/com/liferay/bookmarks`
- `/bookmarks-service/src/main/java/com/liferay/bookmarks`

As a result of having the following tag in the modules `service.xml` file.

`<service-builder package-path="com.liferay.bookmarks">`

In the above example, the `bookmarks-api` module contains the API for the Bookmarks project. All the
classes and interfaces in the `[PROJECT_NAME]-api` module are packaged in a `.jar` file
...
for the portal instance to which your application is deployed.

### The Generated Code [](id=the-generated-code)

Now that we understand how the code is create, where it is created, and why it is organized in that particular way we can take a look at the actual files themselves. Each entity has similar classes generated for it, depending on what you specfied for them in the `service.xml`. 

The amount of code that is generated can be daunting for new developers.  It is important to note that the only modifiable classes are `[ENTITY_NAME]LocalServiceImpl`, `[ENTITY_NAME]ServiceImpl`, `[ENTITY_NAME]Impl`.  The other classes are create either for your to reference, extend, or are used behind the scenes.  Additionally there are some resource files created that can be extended if necessary.  Any code that is placed in a file that is not directly modifiable will be overwritten on subsequent service builder builds. 


+$$$

**Note:** Whenever you add methods to, remove methods from, or change a method signature of a `[ENTITY_NAME]LocalServiceImpl` class, `[ENTITY_NAME]ServiceImpl` class, or `[ENTITY_NAME]Impl` class, you should run Service Builder again to regenerate the affected 
...
- Local Service (generated for an entity only if an entity's `local-service`
  attribute is set to `true` in `service.xml`)
    ...
    - `[ENTITY_NAME]LocalServiceWrapper`: Local service wrapper which implements
      ...
      allows developers to modify the entity's local services.
...
- Remote Service (generated for an entity only if an entity's `remote-service`
  attribute is *not* set to `false` in `service.xml`)
    ...
    - `[ENTITY_NAME]ServiceWrapper`: Remote service wrapper which implements
      ...
      developers to modify the remote entity's services.
...
- Model
    ...
    ![Figure 3: Service Builder generates these model classes and interfaces. Only `EventImpl` allows custom methods to be added to the model layer.](../../../images/service-builder-model-diagram.png)
    
- Resources (these files are created in a resource directory, or subdiretory, in your projects service module.)
    - `service.properties`: This file is used by Service Builder to control schema upgrades and load spring configuration files.  Inside        the file there exists a `include-and-override` property which specifies the location to look for an extension file.
    - `portlet-model-hints.xml`: This is is extensible and used to specific entity data restrictions
    - `module-hbm.xml`: This is a Hibernate mapping file
    - `portlet-model-hints.xml`: This is a Spring file used to configure Spring beans and other Spring related configurations.
    - `tables.sql`: This file holds the create table SQL code generated as a result of  `service.xml` and other files
    - `indexes.xml`: This file holds the indexes which are applied to the tables in  `tables.sql`
    - `sequences.sql`: This file holds the sequence information create by Service Builder
...
Each class file that Service Builder generates is assembled from an associated
...
`[ENTITY_NAME]ServiceImpl.java` file is generated, just look at the
...
[Running Service Builder and Understanding the Generated Code in 6.2](/develop/tutorials/-/knowledge_base/6-2/running-service-builder-and-understanding-the-generated-code)
...
[Customizing model entities with model hints](/develop/tutorials/-/knowledge_base/7-0/customizing-model-entities-with-model-hints)
