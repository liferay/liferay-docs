...
some kind of mass update to a large portion of them. A naive approach might be
...
An actionable dynamic query, unlike a regular dynamic query, does not return a
list of service builder entities. Instead, it uses a pagination strategy to
...
and then perform some processing on each of them, a much better strategy would
be to use an actionable dynamic query to process the users in question. This
way, only small numbers of users would be loaded into memory at a time but all
the users would still be processed.

When you run service builder, actionable dynamic query support is included in
...
Each of these methods is implemented by the `FooLocalServiceBaseImpl` class in the
service module. See [here](https://github.com/liferay/liferay-blade-samples/blob/master/gradle/apps/service-builder/foo-api/src/main/java/com/liferay/blade/samples/servicebuilder/service/FooLocalService.java#L156-L166) and [here](https://github.com/liferay/liferay-blade-samples/blob/master/gradle/apps/service-builder/foo-service/src/main/java/com/liferay/blade/samples/servicebuilder/service/base/FooLocalServiceBaseImpl.java#L247-L330) for details.
...
`FooLocalService.getActionableDynamicQuery()` can be used to obtain a new
actionable dynamic query instance for the `Foo` entity. Once you've obtained an
actionable dynamic query instance, you can use chaining to build up the query
using any of the techniques described above for regular dynamic queries, such
as restrictions or projections. But the whole point of using an *actionable*
dynamic query is to specify an action perform on each entity that results from
the query. To specify such an action, use the
...
Here's an example from a test for Liferay's Bookmarks application:
...
To see examples of configuring indexer and export actionable dynamic queries, see see the Bookmarks application [here](https://github.com/liferay/liferay-portal/blob/master/modules/apps/collaboration/bookmarks/bookmarks-service/src/main/java/com/liferay/bookmarks/service/base/BookmarksEntryLocalServiceBaseImpl.java#L285-L296) and [here](https://github.com/liferay/liferay-portal/blob/master/modules/apps/collaboration/bookmarks/bookmarks-service/src/main/java/com/liferay/bookmarks/service/base/BookmarksEntryLocalServiceBaseImpl.java#L307-L405).
To see an example invocation of an indexable actionable dynamic query, see the `reindexEntries` method of the Bookmarks application's indexer [here](https://github.com/liferay/liferay-portal/blob/master/modules/apps/collaboration/bookmarks/bookmarks-service/src/main/java/com/liferay/bookmarks/search/BookmarksEntryIndexer.java#L155-L210).
