...
<!-- Need to test this section on Liferay 7. -Jesse -->
<!-- Need to update screenshots to Liferay 7. -Jesse -->

![EE Only Feature](../../images/ee-feature-web.png)

The performance, health, and stability of a portal deployment is heavily
dependent on the applications deployed to it. If one application leaks memory
or is extremely slow, your entire portal can slow to a crawl or crash due to a
dreaded `OutOfMemoryError`.

Liferay Portal provides a sandboxing feature that enables you to run
applications in their own container (a.k.a. *sandbox*), reducing any adverse
effects they may have on the health and stability of your portal. The
sandboxing feature is provided by Liferay's Sandbox App. This app lets you
create sandboxes to run portlet applications in separate JVMs, freeing your
portal's JVM from the resource consumption of those portlets. We refer to the
portal's JVM instance as the *Master Portal Instance* (*MPI*) and the sandbox
JVMs as *Slave Portal Instances* (*SPIs*). Since SPIs run on the same host as
the MPI, communication between them is very fast. The fact that sandboxed
portlets are running in SPIs is transparent to your users. Portal users can
continue to use these applications as they normally do.

As a portal administrator, you'll be pleased to know that the Sandboxing app
not only gives you the ability to section off plugins into SPIs, but also gives
you the means to revive an SPI. The Liferay Sandbox App comes with a *SPI
Administration* UI that lets you create, start, stop, and restart SPIs. In
addition, it lets you configure options to automatically restart SPIs that
unexpectedly terminate.

<!-- Need to investigate this paragraph. The first limitation below might not
apply to Liferay 7. -Jesse -->
Liferay's sandboxing feature has two limitations. First, only portlet and web
applications can be deployed on an SPI. Second, the portal ignores SPI portlet
implementation classes that are not remote-safe. Implementation classes (such
as asset renderers and pollers) that register with the portal fall into this
...
<!-- Update link in this paragraph from 6.2 to 7 when the 7.0 Sandboxing App
has been published to Marketplace. -Jesse -->
...
<!-- Update links to docs.liferay.com from 6.2 to 7 when the 7.0 properties doc
is available. -Jesse -->
...
![Figure x: The Sandboxing App comes with an *SPI Administration* UI that's accessible from the portal's Control Panel.](../../images/sandboxing-spi-admin-available-in-control-panel.png)
...
![Figure x: Click the *Add SPI* button to define a new "sandbox" (SPI).](../../images/sandboxing-add-spi-button.png)

The Add SPI panel divides the SPI's fields into General, SPI Configurations, and
Advanced Configurations sections.

![Figure x: You can name your SPI, describe it, and configure it from the *Add SPI* panel.](../../images/sandboxing-add-spi-general-details.png)
...
![Figure x: From the *SPI Runtime* section of your SPI, you can set its maximum number of working threads based the number of threads available to your portal that you'd like to designate for the SPI. You must also set a unique connector port for the SPI.](../../images/sandboxing-configure-spi-runtime.png)
...
![Figure x: All the portlets and web plugins that you've installed on your portal are available for moving into a SPI.](../../images/sandboxing-configure-spi-apps.png)
...
![Figure x: You can specify Java runtime options optimal for your SPI's apps.](../../images/sandboxing-configure-spi-java-runtime.png)
...
![Figure x: You can have the SPI notify you and others if the SPI's apps crash it. And you can configure the maximum number of times to automatically revive the SPI, before requiring manual intervention.](../../images/sandboxing-configure-spi-recovery.png)
...
![Figure x: When you first create a SPI, you'll need to start it manually. You can edit and delete SPIs that are not running.](../../images/sandboxing-start-stop-spi.png)
...
![Figure x: SPI configuration modifications only take affect after the SPI has been restarted.](../../images/sandboxing-modify-spi.png)
...
![Figure x: Click on the global settings gear icon, in the upper right corner of the SPI Administration console, to set default configuration options for all of the portal's SPIs.](../../images/sandboxing-global-settings.png)
...
![Figure x: You can set default notification and restart options for all of the portal's SPIs.](../../images/sandboxing-global-recovery-options.png)
...
![Figure x: Via the SPI Administration's global configuration panel, you can set specific email notification options, including the sender's address, the sender's name, default recipients, a default email subject template, and a default email body template.](../../images/sandboxing-global-notification-options.png)
...
![Figure x: You can set default restart options for your SPIs from the SPI Administration's global configuration panel.](../../images/sandboxing-global-restart-options.png)
...
<!-- Need to populate this section. -Jesse -->
<!-- Add link to WSRP article here. -->
