...
2.  Manually, after downloading the LCS client app's LPKG file to your machine. 
    ...
    **Note**: If an OSGi bundle's changes in the LCS client are internal only 
    and are, therefore, invisible to the OSGi framework, that OSGi bundle stays
    installed and its state information stays unchanged. For example, a bundle
    may contain in-place changes that do not use the API---the framework cannot
    detect such changes. For this reason, deleting the `osgi/state` folder after
    updating the LCS client might be necessary. 
