...
Upgrading to Liferay Portal consists of two steps: upgrading your installation and
then upgrading the database. Liferay Portal can be upgraded using a straightforward
...
`portal-ext.properties` files and decide whether Liferay Portal's new defaults are
...
When a new version of Liferay Portal is released, there are often changes to default
...
to install Liferay on your application server or use Liferay Portal bundled with your
application server of choice. Once you have Liferay Portal installed, DON'T START IT! 
...
time. Liferay Portal instead ships with a tool for upgrading. You must use this tool
...
however, Liferay Portal's modular framework allows you to upgrade modules--even the
core--individually. 
...
`portal-tools-db-upgrade-client`. If you've installed Liferay Portal manually, you
can download the tool as standalone application from the same location where you
downloaded Liferay Portal. Use the following command to start it: 
...
installed new release of Liferay Portal: 
...
use in `portal-ext.properties` to connect Liferay Portal to its database. 
...
When you run the tool, it executes the upgrades and verifiers from Liferay Portal's
...
upgrading Liferay Portal. You can also execute them configuring the [portal properties](http://docs.liferay.com/portal/7.0/propertiesdoc/portal.properties.html#Verify) 
...
After upgrading, you should reindex Liferay Portal's search indexes. Don't just do
this blindly, however: by default, Liferay Portal ships with an embedded
