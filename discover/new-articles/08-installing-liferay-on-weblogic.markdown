...
in a WebLogic Managed server. Deploying to a Managed Server lets you start or
shut down @product@ more quickly and facilitates transitioning @product@ into a
cluster configuration. This article therefore focuses on installing @product@ in
a Managed Server.
...
* `postgre.jar` (if needed) [https://jdbc.postgresql.org/download/postgresql-42.0.0.jar](https://jdbc.postgresql.org/download/postgresql-42.0.0.jar)
...
+$$$
After deploying @product@, you may see an excessive amount of the following warnings and log messages in the console involving the `PhaseOptimizer`. These are benign and can be ignored. Make sure you adjust your app server's logging level or log filters in order to avoid excessive log messages.
