...
dialog box by clicking on the ![Options](../images/icon-app.options.png) in the
portlet's title bar and selecting *Configuration*.<!--deleted references to tabs because there isn't any material here on the Display Settings and RSS tabs-->
...
**Time Format:** Choose *AM/PM* or to *24 Hour*.
...
depends on whether it's being viewed by a guest or a logged-in user. If a user is logged in to the portal, the Calendar portlet displays events using the
...
*Month* view, or choose a more event-oriented *Agenda* setting.
