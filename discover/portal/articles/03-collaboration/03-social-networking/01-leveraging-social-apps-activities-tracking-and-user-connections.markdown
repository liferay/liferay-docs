...
The Activities app displays information about user activity on the site where
you added the app. User activities tracked include updates to the Documents and
Media library, blog posts, message boards posts, wiki pages, and bookmarks.
Liferay also tracks information about web content but only displays this
information if the logged-in user is a site administrator. This application
provides a summary of recent site activity. You can use it on a site's public or
private pages to show what site members have been up to or you can use it on the
public or private pages of a user's personal site. When added to a personal
site, the Activities portlet just shows the activities of a single user.

![Figure 10.2: The Activities app shows information about asset-related user activity in the current site. It only displays information about web content if the current user is a site administrator.](../../images/activities-portlet.png)
...
The User Statistics and Group Statistics apps integrate with Liferay's Social
Activity system. This system tracks user activity and assigns points for various
actions. This lets you find out who are the most active users, the average
number of activities per day, and the highest periods of activity. You'll learn
about this later. 

The Requests app only works on users' personal sites. It lets them respond to
social requests. This app is usually added to a user's private pages,
effectively making it so only that user can respond to his or her social
requests. 

Note that these apps provide a default implementation of Liferay's Social API.
It's also possible to develop your own implementation of Liferay's social API to
use different social relationships. Please refer to the [Liferay Developer's
