...
Adaptive Media is an application on [Liferay Marketplace](https://web.liferay.com/marketplace) 
that delivers your media content in the best format, based on the channel and
device that consumes it. It runs on both Liferay Portal CE 7.0 and Liferay DXP. 
...
device. Adaptive Media generates new images that are scaled to fit the desired
resolutions and maintain their original aspect ratio. 
...
Open the Control Panel and select *Adaptive Media* under the *Configuration*
heading to administer Adaptive Media. 

You must specify the image resolutions to generate when a new image is uploaded.
Once you create a resolution, the system generates it automatically for all
images uploaded, but it won't update images uploaded before you created the new
resolution. To apply a new resolution to previously uploaded images, you must
trigger the process to adapt the remaining images (see the [Generating Missing
Image Resolutions section](#generating-missing-image-resolutions) below). 
