...
1.  Start by copying the default `navigation.ftl` file from the 
    `/src/build/templates/` folder into the theme's `/src/templates` folder. The 
    `/build` folder was generated when you built the theme and again when you 
    initially deployed the theme in the last section.
