...
if [ $# -lt 1 ]; then
	echo
	echo Usage: ./convert.sh article.markdown or ./convert.sh article.markdown article.html
	echo
	echo The first argument, article.markdown, is the path to the Markdown file to convert to HTML.
	echo
	echo The second argument, article.html, is optional. It specifies the path to the HTML file to be created. If this argument is omitted, the HTML file to be created will be created in the same directory as the Markdown file and will have the same filename as the Markdown file except that the .markdown file extension will be replaced by the .html file extension.  
	echo
	exit 1
elif [ $# -lt 2 ]; then
	htmlFile=$1
